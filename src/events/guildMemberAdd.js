...
class GuildMemberAdd extends Event {
  ...
  async run(bot, member) {
    ...
    if (guild.vanityURLCode)
      newInvites.set(guild.vanityURLCode, await guild.fetchVanityData());
    bot.guildInvites.set(guild.id, newInvites);

    const usedInvite = newInvites.find(
      (inv) => inviter.get(inv.code).uses < inv.uses
    );
    // Wasn't able to find the invite
    if (!usedInvite)
      return welcomeChannel
        .send(
          `${member} has joined the server but I do not know how. Perhaps a temporary invite?`
        )
    ...
    if (usedInvite.code === member.guild.vanityURLCode)
      return welcomeChannel.send(
        `${member} has joined the server using the vanity link!`
      );

    if (!welcomeChannel) return;
    const toSend = messages.welcome
      .replace(/\{member\}/g, member.toString())
      .replace(/\{inviter\}/g, usedInvite.inviter.tag)
      .replace(/\{invites\}/g, usedInvite.uses);
